...
X_inter_RNA = ML_data[ML_data['cohort'] == 'inter'].iloc[:,34:44].reset_index(drop=True)
y_inter = ML_data.loc[ML_data['cohort'] == 'inter','label'].reset_index(drop=True)
print(X_inter_RNA.shape)
print( Counter(y_inter) )

X_exter_RNA = ML_data[ML_data['cohort'] == 'exter'].iloc[:,34:44].reset_index(drop=True)
y_exter = ML_data.loc[ML_data['cohort'] == 'exter','label'].reset_index(drop=True)
print(X_exter_RNA.shape)
print( Counter(y_exter) )

scaler = StandardScaler()
X_train = scaler.fit_transform(X_inter_RNA)
X_test = scaler.transform(X_exter_RNA)
...
results_df = pd.DataFrame(results)
results_df.to_csv('./results/final_RNA.csv', index=False)
